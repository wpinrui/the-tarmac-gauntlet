...
// ---------------------------------------------------------------------------
// Person icon SVG (reused for driver avatars)
// ---------------------------------------------------------------------------
const PersonIcon = () => (
  ...
  return (
    <div className="stat-bar-row">
      ...
      <div className="stat-bar-track">
        <div
          className={`stat-bar-fill ${pct < 20 ? "low" : ""}`}
          style={{ width: `${pct}%` }}
        />
      </div>
...
export function GarageScreen() {
  ...
  const carModel: CarModel | undefined = selectedCar
    ...
    : undefined;
  const effectiveStats = selectedCar && carModel
    ? calculateEffectiveStats(selectedCar, carModel)
    : null;
  ...
  const upgradeText = upgradeCount === 0
    ? "No upgrades"
    : `${upgradeCount} upgrade${upgradeCount > 1 ? "s" : ""} installed`;

  // --- Driver data ---
  const playerDriverContracts = game.contracts.filter(
    (c) => c.teamId === player.id && c.remainingYears > 0,
  );
  const hiredDrivers: (Driver & { salary: number })[] = playerDriverContracts
    .map((c) => {
      const d = game.drivers.find((dr) => dr.id === c.driverId);
      return d ? { ...d, salary: c.annualSalary } : null;
    })
    .filter((d): d is Driver & { salary: number } => d !== null);

  const playerCharStats = player.skills.driver * 5;
      ...
      <div className="garage-app">
        ...
        <div className="garage-main">
          ...
          <div className="stat-strip">
            ...
            <div className="stat-chip">
              <div className="chip-icon">
                <svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8z" /></svg>
              </div>
              <div className="chip-text">
          ...
          <div className="card-grid">
                  ...
                  <div className="car-tabs">
                    {cars.map((car, i) => {
                      ...
                      return (
                        <button
                          key={car.id}
                          className={`car-tab ${i === selectedCarIdx ? "active" : ""}`}
                          onClick={() => setSelectedCarIdx(i)}
                        >
                    ...
                    <>
                      ...
                      <div className="car-condition">
                        <div className="condition-row">
                          ...
                          <span className={`condition-pct ${selectedCar.condition < 50 ? "warn" : ""}`}>
            ...
            <div className="card">
              ...
              <div className="driver-slot">
                ...
                <div className="driver-info">
                  ...
                  <div className="driver-detail">
                    All stats: {playerCharStats} / 100 &middot; No salary
                  </div>
                </div>
              </div>
              ...
              {hiredDrivers.map((d) => {
                ...
                return (
                  <div className="driver-slot" key={d.id}>
                    <div className="driver-avatar"><PersonIcon /></div>
                    <div className="driver-info">
                      <div className="driver-name-text">{d.name}</div>
                      <div className="driver-detail">
                        Total stats: {total} &middot; ${d.salary.toLocaleString()}/yr
                      </div>
                    </div>
                  </div>
                );
              ...
              {Array.from({ length: emptySlots }, (_, i) => (
                <div className="empty-slot" key={`empty-${i}`}>
                  <div className="empty-slot-inner">Empty driver slot</div>
                </div>
              ))}
            </div>
            ...
            <div className="card">
              ...
              <div className="skill-item">
                <span className="skill-label">Driver</span>
                <span className="skill-val">{player.skills.driver} <span className="skill-max">/ 20</span></span>
              </div>
              <div className="skill-item">
                <span className="skill-label">Engineer</span>
                <span className="skill-val">{player.skills.engineer} <span className="skill-max">/ 20</span></span>
              </div>
              <div className="skill-item">
                <span className="skill-label">Business</span>
                <span className="skill-val">{player.skills.business} <span className="skill-max">/ 20</span></span>
              </div>
            </div>
          </div>
          ...
          <div className="nav-cards">
            <div className="nav-card">
              <div className="nav-card-icon">
                <svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="13" rx="2" /><line x1="12" y1="16" x2="12" y2="21" /><line x1="8" y1="21" x2="16" y2="21" /></svg>
              </div>
              <span className="nav-card-label">New Car Dealer</span>
            </div>
            <div className="nav-card">
              <div className="nav-card-icon">
                <svg viewBox="0 0 24 24"><path d="M9 11l3-3 3 3M12 8v8" /><rect x="3" y="3" width="18" height="18" rx="2" /></svg>
              </div>
          ...
          <div className="race-row">
            <span className="race-year">
              Year <strong>{game.currentYear}</strong> &middot; The 24h Tarmac Gauntlet
            </span>
            <button className="btn-race" disabled={!player.enteredCarId}>
              Start Race
            </button>
          </div>
        </div>
